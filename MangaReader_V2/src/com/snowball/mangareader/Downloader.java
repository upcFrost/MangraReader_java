...
public class Downloader extends AsyncTask<Long, Integer, Boolean> {
	...
	protected void onProgressUpdate(Integer... values) {
		...
		if (progress == 100) {
			MangaReader.mDbHelper
			.updateChapter(bookTable, chapter_id, null, null,
					StaticValues.DOWNLOADED, -1, -1, 100, -1);
			...
			MangaReader.mDbHelper.updateChapter(bookTable, chapter_id, null, null,
					-1, -1, -1, progress, -1);
		}
		...
		while (true) {
			...
			while (!queue) {
				try {
					...
					MangaReader.mDownloadsCursor = MangaReader.mDbHelper
			...
			manga_id = MangaReader.mDownloadsCursor
					.getLong(MangaReader.mDownloadsCursor
							.getColumnIndex(DbAdapter.KEY_DOWNLOAD_MANGA_ID));
			chapter_id = MangaReader.mDownloadsCursor
					.getLong(MangaReader.mDownloadsCursor
							.getColumnIndex(DbAdapter.KEY_DOWNLOAD_CHAPTER_ID));
			...
			Matcher m;
			Pattern pic = Pattern
					.compile(".*<img src=\"(.*?goodmanga.net/images/manga.*?)[0-9]*.jpg.*?alt=.*?Page (.*?)\".*?>");
			Pattern total = Pattern
					.compile("</select>.*?<span>of ([0-9]*?)</span>");
			String line = null;
			...
			Cursor chapterCursor = MangaReader.mDbHelper.fetchChapterById(
					mangaCursor.getString(mangaCursor
							.getColumnIndex(DbAdapter.KEY_BOOK_TABLE)),
					chapter_id);
			chapterCursor.moveToFirst();

			// Populate database and add manga to favorites
			local_path = StaticValues.mangaDirPath
					+ StaticValues.escapeRE(mangaCursor.getString(mangaCursor
							.getColumnIndex(DbAdapter.KEY_BOOK_TABLE)));
			MangaReader.mDbHelper.updateManga(manga_id, null, null, null, null,
					local_path, null, -1, -1, -1, null, 1, -1);
			bookTable = mangaCursor.getString(mangaCursor
					.getColumnIndex(DbAdapter.KEY_BOOK_TABLE));
			MangaReader.mDbHelper.updateChapter(bookTable, chapter_id, null,
					null, StaticValues.DOWNLOADING, -1, -1, -1, -1);
			//publishProgress(0);

			// Initialize download of the first page
			String url = chapterCursor.getString(chapterCursor
					.getColumnIndex(DbAdapter.KEY_BOOK_URL));
			try {
				...
				if (currentDownloading != StaticValues.DOWNLOADING) {
					MangaReader.mDbHelper.updateChapter(bookTable, chapter_id,
							null, null, currentDownloading, -1, -1,
							-1, -1);
					continue;
				}
				...
				if (m.find()) {
					...
					MangaReader.mDbHelper.updateChapter(bookTable, chapter_id,
							null, null, -1, -1, pageTotal, -1, -1);
					//publishProgress(0);
				} else {
					System.out.println("Downloader: 'total' pattern failed");
					MangaReader.mDbHelper.updateChapter(bookTable, chapter_id,
							null, null, StaticValues.NOT_DOWNLOADING, -1, -1,
							-1, -1);
					publishProgress(0);
					continue;
				}
				...
				String filePath = local_path
						+ "/"
						+ chapterCursor.getInt(chapterCursor
								.getColumnIndex(DbAdapter.KEY_BOOK_NUM)) + "/";
				...
				for (int pageNumber = lastPage; pageNumber <= pageTotal; pageNumber++) {
					...
					if (!downloadURL(downloadURL, pageTotal, pageNumber, filePath,
							fileName) || currentDownloading != StaticValues.DOWNLOADING) {
						downloaded = false;
						break;
					}
					if (currentDownloading == StaticValues.DOWNLOADING) {
